...
if __name__ == '__main__':
    ...
    df = pd.read_csv(
        f'/Users/arnablahiry/repos/SR_project/data/csv/l1norm_training_data_b{N_BINS}.csv'
    )

    # Verify the expected bin column actually exists in the CSV
    assert f'bin_{BIN_IDX}' in df.columns, (
        f"Column 'bin_{BIN_IDX}' not found in CSV. "
        f"Available bin columns: {[c for c in df.columns if c.startswith('bin_')]}\n"
    ...
    y      = df[f'bin_{BIN_IDX}'].to_numpy()             # shape: (n_samples,)
    ...
    X = df[X_names].to_numpy().T                         # shape: (3, n_samples)
