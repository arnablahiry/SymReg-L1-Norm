"""
NOTE: This is config_direct.py — tuned for the DIRECT SR approach
(4 variables: Omega_m_std, sigma_8_std, w_std, x_c → L1-norm value).
Do NOT use for the PCA approach (use config.py instead).
...
MAX_LENGTH = 200  # direct SR needs room: double-lobe shape + cosmo modulation = ~30-60 nodes
...
# → LENGTH_LOC = 30   # direct SR: expressions need ~20-40 nodes — 10 was killing cosmo params : very permissive, SR can explore complex expressions freely.
# → For cosmology L1-norm (expected power-law ~5-15 nodes), try LENGTH_LOC = 8–15.
LENGTH_LOC = 50   # direct SR: expressions need ~20-40 nodes — 10 was killing cosmo params
...
reward_config = {
    ...
    "zero_out_duplicates" : True,   # was False — prevents RNN locking onto one expression with only 156 samples
...
learning_config = {
    ...
    'entropy_weight'   : 0.1,   # raised — 4 vars need more exploration to find cosmo params
...
free_const_opti_args = {
    ...
    'method_args': {
        ...
        'n_steps' : 100, # direct SR: 4-var expressions have complex constant landscapes
...
priors_config = [
    ...
    ("HardLengthPrior", {"min_length": 8, "max_length": MAX_LENGTH}),  # min=8: prevents trivial x_c-only expressions from dominating early
    ...
    # x can appear only 4 times so that the network uses other variables
    ("OccurrencesPrior", {"targets" : ["x",], "max" : [4,] }),
